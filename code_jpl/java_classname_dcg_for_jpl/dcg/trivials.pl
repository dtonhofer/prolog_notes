...
jpl_java_identifier_raw(I) --> { if_then(nonvar(I),atom_codes(I,[C|Cs])) },
                               { jpl_java_identifier_start_char(C) },
                               jpl_java_identifier_part_chars(Cs),
                               { if_then(var(I),atom_codes(I,[C|Cs])) }.

jpl_java_identifier_part_chars([C|Cs]) --> [C], { java_identifier_part_char(C) } ,!, jpl_java_identifier_part_chars(Cs).
...
jpl_nonempty_digits([D|Ds]) --> jpl_digit(D),jpl_nonempty_digits(Ds), !.
jpl_nonempty_digits([D])    --> jpl_digit(D).

% I sure hope the compiler optimizes the decomposition of 0123456789
 
jpl_digit(D) --> [D], { memberchk(D,`0123456789`) }.

...
- There may be two version of output predicates:
   - One taking a (mode +) stream
   - One not taking a stream (i.e. using the globally defined stream `current_output` 
     (see [Predefined stream aliases](https://eu.swi-prolog.org/pldoc/man?section=streamalias)) 
- To capture the ouput to `current_output` generated by a goal, there is
  [`with_output_to/2`](https://eu.swi-prolog.org/pldoc/doc_for?object=with_output_to/2), which captures
  the output in a list of characters, codes, a string or an atom; and the less generic
  [`with_output_to_chars/2`](https://eu.swi-prolog.org/pldoc/doc_for?object=with_output_to_chars/2), 
  which captures the ouput in a list of codes (not chars). Nice!  

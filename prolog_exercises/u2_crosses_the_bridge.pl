% 2019-08-05 ronerycoder@gluino.name
...
solve(HistoryOut) :- explore([state([bono,edge,adam,larry],[],0)],HistoryOut).
...
team_selection(Candidates,[A,B]) :- select(A, Candidates, SmallerCands),select(B, SmallerCands, _), A @< B. 
team_selection(Candidates,[A])   :- select(A, Candidates, _).
